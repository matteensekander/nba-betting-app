...
app.get('/api/teams/:teamId/roster', async (req, res) => {
  try {
    const url = `${ESPN_SITE}/site/v2/sports/basketball/nba/teams/${req.params.teamId}/roster`;
    const data = await espnFetch(url);
...
// Game odds (The Odds API — set ODDS_API_KEY env var)
app.get('/api/odds', async (req, res) => {
  ...
  try {
    ...
    const games = (Array.isArray(data) ? data : []).map(game => {
      const allMoneylines = [], allSpreads = [], allTotals = [];
      ...
      for (const o of allMoneylines) { if (!bestML[o.name] || o.price > bestML[o.name].price) bestML[o.name] = o; }
      for (const o of allSpreads) { if (!bestSpread[o.name] || o.price > bestSpread[o.name].price) bestSpread[o.name] = o; }
      for (const o of allTotals) { const k = `${o.name}_${o.point}`; if (!bestTotal[k] || o.price > bestTotal[k].price) bestTotal[k] = o; }

      const moneylines = Object.values(bestML);
      const totalImplied = moneylines.reduce((s, o) => s + toImplied(o.price), 0);
      const withValue = moneylines.map(o => ({ ...o, implied: Math.round(toImplied(o.price) * 100), value: Math.round((toImplied(o.price) / totalImplied) * 100) }));
...
app.get('/api/players/:id/profile', async (req, res) => {
  try {
    const url = `${ESPN_API}/common/v3/sports/basketball/nba/athletes/${req.params.id}`;
    const data = await espnFetch(url);
    const a = data.athlete || {};
    res.json({ id: req.params.id, name: a.fullName, firstName: a.firstName, lastName: a.lastName, position: a.position?.abbreviation, team: a.team?.displayName, teamAbbr: a.team?.abbreviation, headshot: a.headshot?.href, jersey: a.jersey });
  } catch (err) {
    res.status(err.status || 500).json({ error: err.message });
  }
});
...
app.get('/api/players/:id/gamelog', async (req, res) => {
  try {
    ...
    const url = `${ESPN_API}/common/v3/sports/basketball/nba/athletes/${req.params.id}/gamelog?season=${year}`;
    const data = await espnFetch(url);
    res.json(data);
  } catch (err) {
    res.status(err.status || 500).json({ error: err.message });
